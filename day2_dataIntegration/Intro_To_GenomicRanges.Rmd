...
library(genomation)
library(GenomicRanges)
g_k562 = readGeneric('./Data/CTCF_K562_hg38.chr21.bed')
g_gm   = readGeneric('./Data/CTCF_GM12878_hg38.chr21.bed')
...
```{r coverage_2_ex, exercise=TRUE, exercise.setup='wig_1'}
...
meth = readGeneric('./Data/WGBS_K562_hg38.chr21.bed.gz',zero.based=TRUE, meta.cols=list(score=11))
fom  = as.data.frame(findOverlaps(r, meth))
fom$meth  = meth$score[fom$subjectHits]
fom$class = r$class[fom$queryHits]
...
**Hint:** readGeneric from the genomation package
...
wgbs = lapply(bed_files, function(x)readGeneric(x,zero.based=TRUE, meta.cols=list(score=11)))
...
meth_list = lapply(meth_files, function(x)readGeneric(x,zero.based=TRUE, meta.cols=list(score=11)))
